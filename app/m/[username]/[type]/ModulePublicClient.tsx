...
function AgencyModule({ config, username, profileId, agentData }: {
  ...
  const contactRaw   = String(config.contactPhone || config.contactWhatsapp || '');
  const contactUrl   = contactRaw.startsWith('http')
    ? contactRaw
    : contactRaw ? `https://wa.me/${contactRaw.replace(/\D/g, '')}` : '';
  ...
  const links = [
    ...
    contactUrl   && { icon: '💬', label: contactLabel,                 color: '#10B981', bg: 'rgba(16,185,129,0.12)', border: 'rgba(16,185,129,0.3)', action: 'contact'    as const, url: contactUrl },
